...
<div class="message" :class="bg">
  <p class="message_sucсsess" v-if="result.message">Правильно!</p>
  <div class="message_error" v-else>Ошибка! <br> {{operands.one}} {{operationTarget}}
    <span v-if="operands.two >= 0">{{operands.two}}</span>
    <span v-else>( {{operands.two}} )</span>
      = {{resultCalc}} !</div>
  <hr>
  <button class="message_button" type="button" @click="btnClick">Дальше</button>
</div>   
</template>
